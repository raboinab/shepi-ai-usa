...
serve(async (req) => {
  ...
  try {
    ...
    // Fetch processed_data for ALL proof documents (not just payroll)
    let allProcessedData: Record<string, unknown>[] = [];
    {
      ...
      if (pdRows && pdRows.length > 0) {
        allProcessedData = pdRows;
        console.log(`Found ${pdRows.length} processed_data rows for proof documents`);
      }
    }
    ...
    const documentContext = documents.map(doc => {
      ...
      if (doc.account_type) content += ` | Account Type: ${doc.account_type}`;
      
      // Find matching processed_data for this document
      const matchingPd = allProcessedData.find(
        (pd: any) => pd.source_document_id === doc.id
      );

      if (matchingPd) {
        const extracted = (matchingPd as any).data?.extractedData || (matchingPd as any).data;
        const dataType = (matchingPd as any).data_type || '';
        
        // Payroll-specific formatting for readability
        if (dataType === 'payroll_register' || doc.account_type === 'payroll' || doc.category === 'payroll') {
          if (extracted) {
            const sections = ['salaryWages', 'ownerCompensation', 'payrollTaxes', 'benefits'];
            for (const section of sections) {
              if (extracted[section] && Array.isArray(extracted[section])) {
                content += `\n\n${section} LINE ITEMS:`;
                for (const item of extracted[section]) {
                  const name = item.name || item.description || item.employee_name || 'Unknown';
                  const monthlyValues = item.monthlyValues || {};
                  const total = Object.values(monthlyValues).reduce((s: number, v: any) => s + (Number(v) || 0), 0);
                  const months = Object.keys(monthlyValues);
                  content += `\n  - ${name}: Total $${total.toLocaleString()} (${months.length} periods: ${months.slice(0, 3).join(', ')}${months.length > 3 ? '...' : ''})`;
                }
              }
            }
            if (typeof extracted.salaryWages === 'number' || typeof extracted.ownerCompensation === 'number') {
              content += `\nPayroll Totals: salaryWages=$${extracted.salaryWages || 0}, ownerCompensation=$${extracted.ownerCompensation || 0}`;
            }
          }
        } else {
          // Generic: serialize the extracted data as JSON (truncated)
          const dataStr = JSON.stringify(extracted);
          content += `\nExtracted Data (${dataType}): ${dataStr.slice(0, 3000)}`;
          if (dataStr.length > 3000) content += '... [truncated]';
        }
      } else if (doc.parsed_summary) {
        // Fallback to parsed_summary when no processed_data exists
...
Analyze the above adjustment and supporting documentation. Provide your validation assessment as JSON.`;
    ...
    const proofData = {
      ...
      traceability_data: { document_ids: documentIds },
    };
