...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Aggregate trial_balance records — but only when we actually have a COA to match against.
    // Without a COA the resolver mis-links accounts that share a leaf name.
    ...
    try {
      console.log("[complete-qb-sync] Triggering embed-project-data...");
      ...
      }).catch((e) => console.error("[complete-qb-sync] embed-project-data fire-and-forget error:", e));
    } catch (e) {
      console.error("[complete-qb-sync] Failed to trigger embed-project-data:", e);
    }
...
function aggregateTrialBalanceRecords(
  ...
  console.log(
    `[aggregateTrialBalance] Processing ${records.length} records, COA has ` +
      `${coaMaps.byId.size} by id, ${coaMaps.byRealNumber.size} by real#, ` +
      `${coaMaps.byFqn.size} by fqn, ${coaMaps.byName.size} by name`,
  );
  ...
  for (const record of records) {
    ...
    for (const row of rows) {
      ...
      // QB raw report: colData[0].id IS the QB account id, not an account number.
