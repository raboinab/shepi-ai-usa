...
Extract everything you can see. If the document is unclear or has limited content, extract what's available and note any limitations in the summary.`;
    ...
    if (['pdf', 'jpg', 'jpeg', 'png', 'webp'].includes(fileType)) {
      ...
      // Parse Excel binary files into readable text
      try {
        ...
        console.log(`Parsed Excel: ${workbook.SheetNames.length} sheets, ${excelText.length} chars`);
        userContent.push({
          type: 'text',
          text: `Please extract and structure the key information from this Excel document named "${document.name}":\n\n${excelText}`
        });
      } catch (xlsError) {
        console.error('Failed to parse Excel file:', xlsError);
        userContent.push({
          type: 'text',
          text: `Unable to parse Excel document "${document.name}". File type: ${fileType}`
        });
      }
    } else {
      // For non-image, non-Excel files, try to decode as text
      try {
