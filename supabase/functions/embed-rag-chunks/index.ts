...
serve(async (req) => {
  ...
  try {
    ...
    // Parse body once for both delete and embed paths
    const body = await req.json();

    // Handle DELETE: remove all chunks for a given source
    if (req.method === "DELETE" || body.action === "delete") {
      ...
      if (error) {
        console.error(`Error deleting chunks for source ${source}:`, error);
        throw error;
      }

      console.log(`Deleted ${count} chunks for source: ${source}`);
      return new Response(
        JSON.stringify({ success: true, deleted: count, source }),
    ...
    const { chunks, source, source_title, source_license, authority_weight } = body as {
      chunks: ChunkInput[];
      source: string;
      source_title?: string;
      source_license?: string;
