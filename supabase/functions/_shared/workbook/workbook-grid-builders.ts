/**
 * Pure grid-building functions (server-side copy).
 * Mirrors src/lib/workbook-grid-builders.ts — keep in sync.
 */
import type { DealData, GridData, GridRow, GridColumn } from "./workbook-types.ts";
import * as calc from "./calculations.ts";
import {
  buildStandardColumns, periodCells, negatedPeriodCells, marginCells,
  entryCells, adjustmentCells, bsEndingBalanceCells, negatedBsEndingBalanceCells, bsEntryCells,
} from "./tabHelpers.ts";
import { buildBreakdownGrid } from "./breakdownGridBuilder.ts";
...
export function buildIncomeStatementGrid(dealData: DealData): GridData {
  ...
  const opex = (p: string) => calc.calcOpEx(tb, p) + calc.sumReclassImpact(rc, "Operating expenses", p);
  const payroll = (p: string) => calc.calcPayroll(tb, p) + calc.sumReclassImpact(rc, "Payroll & Related", p);
...
export function buildPayrollGrid(dealData: DealData): GridData {
  ...
  if (!tbHasData && (dealData as Record<string, unknown>).payrollFallback) {
    return buildPayrollFallbackGrid(dealData, (dealData as Record<string, unknown>).payrollFallback as PayrollFallback);
  }
...
function buildPayrollFallbackGrid(dealData: DealData, fb: PayrollFallback): GridData {
  const periods = dealData.deal.periods;
...
export function buildNWCAnalysisGrid(dealData: DealData): GridData {
  ...
  const rows: GridRow[] = [
    ...
    { id: "trail-ar", type: "data", cells: { label: "A/R Days Outstanding", ...bc(p => {
      const ar = Math.abs(calc.sumByLineItem(tb, "Accounts receivable", p));
      const rev = Math.abs(calc.calcRevenue(tb, p));
      return rev < 1 ? 0 : (ar / rev) * 30;
    }) } },
    { id: "trail-ap", type: "data", cells: { label: "A/P Days Outstanding", ...bc(p => {
      const ap = Math.abs(calc.sumByLineItem(tb, "Current liabilities", p));
      const cogs = Math.abs(calc.calcCOGS(tb, p));
