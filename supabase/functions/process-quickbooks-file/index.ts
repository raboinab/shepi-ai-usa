...
function deriveCoaFromGlData(glData: unknown): DerivedAccount[] {
  ...
  // Auto-assign account numbers to accounts missing them
  let nextAutoNumber = 10000;
  for (const acc of accounts) {
    if (!acc.accountNumber) {
      while (accounts.some(a => a.accountNumber === String(nextAutoNumber))) {
        nextAutoNumber++;
      }
      acc.accountNumber = String(nextAutoNumber);
      nextAutoNumber++;
    }
  }
...
serve(async (req) => {
  ...
  try {
    ...
    // Detect empty trial balance parse and fail early
    if (category === 'trial_balance' && recordCount === 0) {
      console.error('[process-quickbooks-file] Trial balance parsed with 0 records — marking as failed');
      await supabase
        .from('documents')
        .update({
          processing_status: 'failed',
          parsed_summary: { error: 'Trial balance file contained no parseable monthly data. Check the file format or date range.' }
        })
        .eq('id', documentId);

      return new Response(JSON.stringify({
        error: 'Trial balance file contained no parseable data',
        documentId,
