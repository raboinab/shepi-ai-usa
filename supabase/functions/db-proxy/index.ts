...
serve(async (req) => {
  ...
  const validApiKeys = [
    ...
    Deno.env.get("DISCOVERY_API_KEY"),
  ].filter(Boolean) as string[];

  if (validApiKeys.length === 0) {
    console.error("No API keys configured - check QB_AUTH_API_KEY, SHEPI_SHEETS_API_KEY, DOCUCLIPPER_API_KEY, QBTOJSON_API_KEY, or DISCOVERY_API_KEY");
      return new Response(
    ...
    switch (action) {
      case "query": {
        ...
        // Response size guard — prevent 546 crashes on large payloads
        if (result) {
          const serialized = JSON.stringify(result);
          const sizeBytes = new TextEncoder().encode(serialized).length;
          const sizeMB = sizeBytes / (1024 * 1024);
          if (sizeMB > 4) {
            console.error(`[${serviceName}] Response too large: ${sizeMB.toFixed(2)}MB (${Array.isArray(result) ? result.length : 1} rows) on table=${table}`);
            return new Response(
              JSON.stringify({
                error: "Payload Too Large",
                message: `Response is ${sizeMB.toFixed(2)}MB which exceeds the 4MB limit. Use direct Postgres connection for large reads.`,
