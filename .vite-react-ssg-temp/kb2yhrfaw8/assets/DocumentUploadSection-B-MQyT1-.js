...
import { T as TooltipProvider, o as Tooltip, p as TooltipTrigger, n as cn, q as TooltipContent, j as Badge, s as supabase, B as Button, C as Card, b as CardHeader, d as CardTitle, f as CardContent, e as CardDescription, w as parseLocalDate } from "../main.mjs";
import { I as Input } from "./input-RFtselAh.js";
import { L as Label } from "./label-DiNTC6Nb.js";
import { S as Select, a as SelectTrigger, b as SelectValue, c as SelectContent, e as SelectGroup, f as SelectLabel, d as SelectItem } from "./select-WnCLmZWE.js";
...
import { C as CIMInsightsCard } from "./CIMInsightsCard-DA5cWsp1.js";
...
import { D as DocumentValidationDialog } from "./DocumentValidationDialog-D9RjkW7A.js";
import { D as DocumentChecklistReference } from "./DocumentChecklistReference-C6te8ZOj.js";
...
const DocumentUploadSection = ({
  ...
  const fetchGLAnalysis = async () => {
    try {
      const { data: data2, error } = await supabase.from("processed_data").select("id, data, created_at").eq("project_id", projectId).eq("data_type", "general_ledger_analysis").order("created_at", { ascending: false }).limit(1);
      if (error) throw error;
      if (data2 && data2.length > 0) {
  ...
  const fetchJEAnalysis = async () => {
    try {
      const { data: data2, error } = await supabase.from("processed_data").select("id, data, created_at").eq("project_id", projectId).eq("data_type", "journal_entry_analysis").order("created_at", { ascending: false }).limit(1);
      if (error) throw error;
      if (data2 && data2.length > 0) {
