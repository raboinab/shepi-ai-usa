...
import { u as useSEO, B as Button, a as clearOAuthProcessedFlag, h as hasOAuthCallback, s as supabase, g as trySetSessionFromUrlHash, c as cleanupOAuthHash, i as trackEvent } from "../main.mjs";
...
function fireAuthEvent(user, fallbackMethod = "unknown") {
  if (!user.created_at || !user.last_sign_in_at) return;
  const created = new Date(user.created_at).getTime();
  const lastSignIn = new Date(user.last_sign_in_at).getTime();
  const isNewUser = Math.abs(lastSignIn - created) < 1e4;
