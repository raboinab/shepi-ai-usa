...
import { aD as buildStandardColumns, aw as negatedPeriodCells, ax as periodCells, N as sumByLineItem, a2 as calcTotalCurrentLiabilities, aH as bsEndingBalanceCells, aL as calcUndepositedFunds, F as calcRevenue, aj as calcCOGS, ak as calcOpEx, al as calcPayroll, an as calcDepreciationExpense, am as calcOtherExpense, H as calcNetIncome } from "./sanitizeWizardData-Dv9tWNGG.js";
import { u as useProofOfCashData, a as useTransferClassification, T as TransferReviewDialog } from "./TransferReviewDialog-MNwGzy5h.js";
import { B as Button, j as Badge } from "../main.mjs";
    ...
    !isMock && !classifications && /* @__PURE__ */ jsxs("div", { className: "flex items-center gap-3 px-4 py-2 border-b border-border bg-muted/30", children: [
      ...
      /* @__PURE__ */ jsx(
        Button,
        {
          ...
          variant: "outline",
          onClick: () => {
            import("../main.mjs").then((n) => n.x).then(({ toast }) => toast({ title: "Classification started" }));
            classify().catch((e) => {
              import("../main.mjs").then((n) => n.x).then(({ toast }) => toast({ title: "Classification failed", description: e?.message || "Unknown error", variant: "destructive" }));
            });
          },
    ...
    !isMock && (classifications || liveClassifications) && /* @__PURE__ */ jsxs("div", { className: "flex items-center gap-3 px-4 py-2 border-b border-border bg-accent/30", children: [
      ...
      /* @__PURE__ */ jsx(
        Button,
        {
          ...
          variant: "ghost",
          onClick: () => {
            import("../main.mjs").then((n) => n.x).then(({ toast }) => toast({ title: "Re-run started" }));
            classify().catch((e) => {
              import("../main.mjs").then((n) => n.x).then(({ toast }) => toast({ title: "Re-run failed", description: e?.message || "Unknown error", variant: "destructive" }));
            });
          },
          disabled: isClassifying,
          className: "gap-1.5",
