...
const corsHeaders = {
 ...
 Deno.serve(async (req) => {
   ...
   try {
     ...
     const { project_id, analysis_type = 'full' } = await req.json();
     ...
     console.log(`Analyzing transactions for project ${project_id}, type: ${analysis_type}`);
 
     const { data: processedData, error: fetchError } = await supabase
       .from('processed_data')
     ...
     if (fetchError) {
       console.error('Error fetching processed data:', fetchError);
       return new Response(
     ...
     if (!processedData || processedData.length === 0) {
      ...
      // Extract and build correlation index
      const canonTxns = extractAllTransactions(processedData);
      console.log(`Extracted ${canonTxns.length} transactions from ${processedData.length} records`);
  
      const correlationIndex = buildCorrelationIndex(canonTxns);
      ...
      const flaggedTransactions = await analyzeWithGLFirst(correlationIndex, project_id, user.id, analysis_type, industry);
      console.log(`Flagged ${flaggedTransactions.length} potential adjustments`);
     ...
     return new Response(
       JSON.stringify({
         ...
         message: `Analysis complete. Found ${flaggedTransactions.length} potential adjustments.`
       }),
       { status: 200, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
     );
 ...
 function extractAllTransactions(processedData: any[]): CanonTxn[] {
   const transactions: CanonTxn[] = [];
   ...
   for (const record of processedData) {
     ...
     let extracted: CanonTxn[] = [];
     
     switch (record.data_type) {
       case 'general_ledger':
         extracted = data?.rows?.row 
           ? extractQBGeneralLedger(data, recordId, periodEnd)
           : extractLegacyGL(data, recordId, periodEnd);
         break;
       case 'bank_transactions':
         extracted = extractDocuClipperTransactions(data, recordId, 'bank_transactions');
         break;
       case 'credit_card_transactions':
         extracted = extractDocuClipperTransactions(data, recordId, 'credit_card_transactions');
         break;
       case 'journal_entries':
       case 'deposits':
       case 'bills':
       case 'bill_payments':
         if (data?.rows?.row) extracted = extractQBTransactionReport(data, recordId, record.data_type, periodEnd);
         break;
       case 'trial_balance':
         extracted = extractLegacyTrialBalance(data, recordId, periodEnd);
         break;
       case 'income_statement':
       case 'balance_sheet':
         extracted = extractLegacyFinancialStatement(data, recordId, record.data_type, periodEnd);
         break;
     }
     
     if (extracted.length > 0) {
       transactions.push(...extracted);
       countsBySource[record.data_type] = (countsBySource[record.data_type] || 0) + extracted.length;
     }
   }
 ...
 function isNearPeriodEnd(txDate: string, periodEnd: string): boolean {
  ...
  async function callAI(messages: Array<{role: string; content: string}>): Promise<string | null> {
    ...
    if (!LOVABLE_API_KEY) {
      console.warn("LOVABLE_API_KEY not configured, falling back to deterministic analysis");
      return null;
    }
...
Return ONLY the JSON array.`;
  ...
  async function analyzeWithGLFirst(
    ...
    const hasAIKey = !!Deno.env.get("LOVABLE_API_KEY");

    // ── STEP 1: GL ANALYSIS ──
    if (hasAIKey && index.glEntries.length > 0) {
      console.log(`AI mode: analyzing ${index.glEntries.length} GL entries in batches of ${BATCH_SIZE}`);
      
      // Batch GL entries
      for (let i = 0; i < index.glEntries.length; i += BATCH_SIZE) {
    ...
    if (hasAIKey && bankCCEntries.length > 0) {
      ...
      for (let i = 0; i < bankCCEntries.length; i += BATCH_SIZE) {
        const batch = bankCCEntries.slice(i, i + BATCH_SIZE);
        try {
