...
export function TrialBalanceTab({ dealData }: TabProps) {
  const gridData = useMemo((): GridData => {
    ...
    const columns = [
      { key: "fsType", label: "FS", width: 40, frozen: true, format: "text" as const },
      { key: "accountId", label: "Acct #", width: 70, frozen: true, format: "text" as const },
      { key: "accountName", label: "Account Name", width: 200, frozen: true, format: "text" as const },
      { key: "fsLineItem", label: "FS Line Item", width: 150, frozen: true, format: "text" as const },
    ...
    const isRow: GridRow = {
      ...
      type: "subtotal",
      cells: {
        fsType: "", accountId: "", accountName: "IS Total", fsLineItem: "",
        ...Object.fromEntries(periods.map(p => [p.id, isTotals[p.id] || 0])),
      },
    };
    ...
    for (const p of periods) {
      const checkVal = (bsTotals[p.id] || 0) + (isTotals[p.id] || 0);
