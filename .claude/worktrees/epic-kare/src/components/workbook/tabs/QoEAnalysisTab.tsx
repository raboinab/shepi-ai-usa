...
export function QoEAnalysisTab({ dealData }: TabProps) {
  const gridData = useMemo((): GridData => {
    ...
    const rows: GridRow[] = [
      // Revenue: credit in TB = negative → negate for display
      ...
      { id: "revenue-adjusted", type: "data", cells: { label: "Revenue, as adjusted", ...ec,
        // Reported revenue (negative in TB) minus revenue-targeted adjustments (positive addbacks)
        // npc flips to positive display; subtracting positive adj from negative reported makes it more negative → larger positive display
        ...npc(p => calc.calcRevenue(tb, p) - calc.calcRevenueAdjustments(adj, tbIdx, "all", p)),
      } },
      // Gross Profit: credit-biased in TB when profitable → negate
      { id: "gross-profit", type: "data", cells: { label: "Gross profit, as adjusted", ...ec,
        ...npc(p => calc.calcGrossProfit(tb, p) - calc.calcRevenueAdjustments(adj, tbIdx, "all", p) - calc.calcCOGSAdjustments(adj, tbIdx, "all", p)),
      } },
      // Net Income: IS sum is negative when profitable → negate
      ...
      { id: "ma-ebitda", type: "total", cells: {
        ...
        // Adjustments are positive addbacks; in TB space EBITDA is negative, so subtract adj to increase magnitude, then npc flips positive
        ...npc(p => calc.calcReportedEBITDA(tb, p, ab) - calc.calcAdjustmentTotal(adj, "MA", p)),
      } },
      ...
      { id: "dd-ebitda", type: "total", cells: {
        ...
        ...npc(p => calc.calcReportedEBITDA(tb, p, ab) - calc.calcAdjustmentTotal(adj, "MA", p) - calc.calcAdjustmentTotal(adj, "DD", p)),
      } },
      ...
      { id: "pf-ebitda", type: "total", cells: {
        ...
        ...npc(p => calc.calcReportedEBITDA(tb, p, ab) - calc.calcAdjustmentTotal(adj, "all", p)),
      } },
      ...
      { id: "adj-margin", type: "data", format: "percent", cells: {
        ...
        ...mc(p => -(calc.calcReportedEBITDA(tb, p, ab) - calc.calcAdjustmentTotal(adj, "all", p)), p => calc.calcRevenue(tb, p)),
      } },
