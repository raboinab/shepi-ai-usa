...
export const DD_CHECKLIST_BASE: ChecklistItem[] = [
  ...
  // TIER 2: RECOMMENDED (7) - Standard professional QoE analysis
  ...
  { id: "payrollReports", label: "Payroll Reports/Registers", tier: "recommended", docType: "payroll", needsPeriodCoverage: false, canBeNA: true, checkWizardSection: "payroll", sectionNav: { phase: 3, section: 8, label: "Payroll" } },
  ...
  { id: "balanceSheet", label: "Balance Sheet (for verification)", tier: "recommended", docType: "balance_sheet", needsPeriodCoverage: false, canBeNA: true, checkWizardSection: "balanceSheet", sectionNav: { phase: 5, section: 2, label: "Balance Sheet Report" } },
  { id: "incomeStatement", label: "Income Statement (for verification)", tier: "recommended", docType: "income_statement", needsPeriodCoverage: false, canBeNA: true, checkWizardSection: "incomeStatement", sectionNav: { phase: 5, section: 1, label: "Income Statement Report" } },
  
  // TIER 3: OPTIONAL (7) - Enhanced/situational analysis
  ...
  { id: "journalEntries", label: "Journal Entries", tier: "optional", docType: "journal_entries", needsPeriodCoverage: false, canBeNA: true, checkWizardSection: "journalEntries", sectionNav: { phase: 3, section: 3, label: "Journal Entries" } },
  { id: "creditCardStatements", label: "Credit Card Statements", tier: "optional", docType: "credit_card", needsPeriodCoverage: true, canBeNA: true, checkWizardSection: null, sectionNav: { phase: 2, section: 3, label: "Documents" } },
  { id: "customerList", label: "Top Customers List", tier: "optional", docType: "customer_concentration", needsPeriodCoverage: false, canBeNA: true, checkWizardSection: "topCustomers", sectionNav: { phase: 4, section: 1, label: "Top Customers" } },
  { id: "vendorList", label: "Top Vendors List", tier: "optional", docType: "vendor_concentration", needsPeriodCoverage: false, canBeNA: true, checkWizardSection: "topVendors", sectionNav: { phase: 4, section: 2, label: "Top Vendors" } },
  { id: "inventory", label: "Inventory Listing", tier: "optional", docType: "inventory", needsPeriodCoverage: false, canBeNA: true, checkWizardSection: "inventory", sectionNav: { phase: 3, section: 7, label: "Inventory" }, conditional: true },
  { id: "debtSchedule", label: "Debt Schedule", tier: "optional", docType: "debt_schedule", needsPeriodCoverage: false, canBeNA: true, checkWizardSection: "supplementary", sectionNav: { phase: 3, section: 9, label: "Supplementary" } },
  { id: "contracts", label: "Material Contracts", tier: "optional", docType: "material_contract", needsPeriodCoverage: false, canBeNA: true, checkWizardSection: "materialContracts", sectionNav: { phase: 3, section: 10, label: "Material Contracts" } },
];

// Get filtered checklist based on settings
export function getFilteredChecklist(inventoryEnabled: boolean): ChecklistItem[] {
  return DD_CHECKLIST_BASE.filter(
    item => !item.conditional || (item.id === 'inventory' && inventoryEnabled)
  );
}
...
export function isChecklistItemComplete(
  ...
  if (item.docType) {
    // For period-coverage items: only mark complete if wizardData has coverageComplete=true
    // (used by demo mode to simulate full coverage) or an actual document exists
    if (item.needsPeriodCoverage) {
      // Allow demo/wizard override: wizardData[sectionKey]?.coverageComplete
      if (item.checkWizardSection) {
    ...
    if (documents.some(d => d.account_type === item.docType && d.processing_status === 'completed')) {
      return true;
    }
    // Check processed_data types (e.g. QB-synced journal_entries)
    if (processedDataTypes?.has(item.docType)) {
      return true;
    }
  }
