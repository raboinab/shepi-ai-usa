...
export const ProofValidationBadge = ({ score, status, keyFindings = [], redFlags = [], matchCount, contradictionCount, varianceAmount, onClick }: ProofValidationBadgeProps) => {
  ...
  const hasStats = matchCount !== undefined || contradictionCount !== undefined;
    ...
    <div className="space-y-2 max-w-xs">
      ...
      {hasStats && (
        <div className="text-xs text-muted-foreground">
          {matchCount !== undefined && <span>{matchCount} matches</span>}
          {matchCount !== undefined && contradictionCount !== undefined && <span> · </span>}
          {contradictionCount !== undefined && <span>{contradictionCount} contradictions</span>}
        </div>
      )}
      {varianceAmount != null && varianceAmount !== 0 && (
        <div className="text-xs text-amber-600 font-medium flex items-center gap-1">
          <AlertTriangle className="h-3 w-3" />
          Verified amount differs by {new Intl.NumberFormat("en-US", { style: "currency", currency: "USD", minimumFractionDigits: 0 }).format(Math.abs(varianceAmount))}
        </div>
      )}
