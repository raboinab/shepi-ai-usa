...
export function ARAgingTab({ dealData }: TabProps) {
  const gridData = useMemo((): GridData => {
    const columns = [
      { key: "label", label: "Customer", width: 200, frozen: true, format: "text" as const },
    ...
    const rows: GridRow[] = [];
    const agingPeriods = Object.keys(dealData.arAging).sort();
    if (agingPeriods.length === 0) {
      ...
      for (const periodKey of agingPeriods) {
        const entries = dealData.arAging[periodKey];
        const label = formatAgingPeriodLabel(periodKey);
        rows.push({ id: `hdr-${periodKey}`, type: "section-header", label, cells: { label } });
        for (const [i, e] of entries.entries()) {
          rows.push({ id: `ar-${periodKey}-${i}`, type: "data", editable: true, cells: { label: e.name, current: e.current, d30: e.days1to30, d60: e.days31to60, d90: e.days61to90, d90p: e.days90plus, total: e.total } });
        }
        if (entries.length === 0) {
          ...
          rows.push({ id: `total-${periodKey}`, type: "total", cells: { label: "Total AR", ...totals } });
        }
