...
function computeNarrative(rawData: string[][]): string {
  ...
  const nwc = parseVal(findRow("net working capital")?.[lastCol]) ?? parseVal(findRow("nwc")?.[lastCol]);
  const ca = parseVal(findRow("total current assets")?.[lastCol]) ?? parseVal(findRow("current assets")?.[lastCol]);
  const cl = parseVal(findRow("total current liabilities")?.[lastCol]) ?? parseVal(findRow("current liabilities")?.[lastCol]);

  const parts: string[] = [];
  if (nwc !== null) {
    const str = Math.abs(nwc) >= 1_000_000 ? `$${(Math.abs(nwc) / 1_000_000).toFixed(1)}M` : `$${(Math.abs(nwc) / 1_000).toFixed(0)}K`;
    parts.push(`Net working capital of ${nwc < 0 ? "(" + str + ")" : str}`);
  }
  if (ca !== null && cl !== null && Math.abs(cl) > 0) {
    const ratio = (ca / Math.abs(cl)).toFixed(2);
    parts.push(`current ratio of ${ratio}x`);
  }
  if (parts.length === 0) return "";
  return parts.join(" with ") + ". Review working capital components below for seasonal patterns and normalization considerations.";
}
