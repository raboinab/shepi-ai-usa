...
function computeNarrative(rawData: string[][]): string {
  ...
  const totalAssets = parseVal(findRow("total assets")?.[lastCol]);
  const totalEquity = parseVal(findRow("total equity")?.[lastCol]);

  const parts: string[] = [];
  if (totalAssets !== null) {
    const str = totalAssets >= 1_000_000 ? `$${(totalAssets / 1_000_000).toFixed(1)}M` : `$${(totalAssets / 1_000).toFixed(0)}K`;
    parts.push(`Total assets of ${str}`);
  }
  if (totalEquity !== null && totalAssets) {
    const pct = ((Math.abs(totalEquity) / totalAssets) * 100).toFixed(0);
    parts.push(`equity represents ${pct}% of total assets`);
  }
  if (parts.length === 0) return "";
  return parts.join("; ") + ". Review balance sheet composition below for period-over-period changes.";
}
