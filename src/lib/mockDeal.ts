  ...
  for (let y = 2022; y <= 2024; y++) {
    for (let m = 1; m <= 12; m++) {
      ...
      periods.push({
        ...
        startDate: `${id}-01`,
        endDate: (() => { const ld = new Date(y, m, 0); return `${y}-${String(m).padStart(2, '0')}-${String(ld.getDate()).padStart(2, '0')}`; })(),
      });
    }
  }
  return periods;
...
export function createMockTransferClassifications(): Map<string, { interbank: number; interbankIn: number; interbankOut: number; owner: number }> {
  const periods = buildPeriods();
  const map = new Map<string, { interbank: number; interbankIn: number; interbankOut: number; owner: number }>();
  for (const p of periods) {
