...
import { TAB_GRID_BUILDERS, buildProofOfCashGrid, buildDataSourcesGrid, fetchDocumentSources } from "./workbook-grid-builders";
...
// fetchProofOfCashBankData is now imported from ./fetchPocBankData
...
export async function exportWorkbookXlsx({ dealData, projectId, filename, onProgress }: ExportOptions): Promise<void> {
  ...
  // Enrich dealData with priorBalances if not already present
  ...
  // Pre-fetch bank data for Proof of Cash if projectId available
